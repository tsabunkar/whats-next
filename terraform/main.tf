...
# IAM role for API Gateway to send messages directly to SQS
resource "aws_iam_role" "api_gateway_sqs" {
  name = "${var.project_name}-api-gateway-sqs"
...
# Method response for override case (repo doesn't match → 200)
...
# Method response for success case (queued → 202)
...
# API Gateway integration with SQS (direct service integration)
resource "aws_api_gateway_integration" "webhook" {
  ...
  type                    = "AWS"
  uri                     = "arn:aws:apigateway:${var.aws_region}:sqs:path/${data.aws_caller_identity.current.account_id}/${aws_sqs_queue.webhook_sync.name}"
...
#set($tag = 'latest')
#end
Action=SendMessage&MessageBody=$util.urlEncode("{""repo_name"":""$repoName"",""tag"":""$tag""}")
VTL
  }
}

# Integration response: SQS 200 → client 202
...
resource "aws_api_gateway_deployment" "webhook" {
  depends_on = [
    ...
    aws_api_gateway_method_response.webhook_202,
